...
DEFAULT_POLICY_PROMPT = (
    "Given an Instruction and an optional Input, produce the best natural-language answer.\n"
    "### Instruction:\n{instruction}\n"
    "### Input:\n{input}\n"
    "### Answer:"
)
...
DEFAULT_JUDGE_PROMPT = (
    ...
    "Assign an objective score in [0,1] per the criteria below.  Do not inflate or deflate.\n"
    "### Instruction:\n{instruction}\n"
    "### Input:\n{input}\n"
    "### Reference Answer:\n{reference}\n"
    "### Model Answer:\n{prediction}\n"
    "### Grading Criteria (weights):\n"
    "- Correctness and completeness w.r.t. the Instruction (0.60).\n"
    "- Consistency with the Reference; alternate valid wording or method that still fully satisfies the Instruction should score high (0.25).\n"
    "- Compliance with explicit constraints (format, language, length) (0.10).\n"
    "- Safety (no harmful/illegal/private data) (0.05).\n"
    "### Calibration Guide:\n"
    "1.00 exact and fully compliant.\n"
    "0.80–0.99 minor issues only.\n"
    "0.50–0.79 partially correct or missing key elements.\n"
    "0.20–0.49 mostly incorrect or noncompliant.\n"
    "0.00–0.19 wrong, off-topic, or unsafe.\n"
    "### Output Rule:\n"
    "Return ONLY a number in [0,1] with up to three decimals. No text or symbols.\n"
    "### Score:" 
)


@register_selector('nice_judge')
class NICEJudgeSelector(Selector):
    def __init__(self, 
                 dataset,
                 eval_dataset,
                 accelerator,
        ...
        """初始化 NICEJudgeSelector，加载所需的本地模型并保存超参数。"""
        ...
        logger.info(f"NICEJudgeSelector initialized. Projected gradients will be saved in {self.cache_dir}")

        self._load_local_models()
        logger.info(f"NICEJudgeSelector The policy model has been loaded.: {policy_model_path}")
        logger.info(f"NICEJudgeSelector The judge model has been loaded.: {judge_model_path}")
    ...
    def _load_local_models(self):
        ...
        self.judge_tokenizer = AutoTokenizer.from_pretrained(self.judge_model_path, trust_remote_code=True)
        ...
        self.judge_model.requires_grad_(False)
    ...
    def _format_judge_prompt(self, example: Dict, prediction: str) -> str:
        """构造judge模型使用的提示词。"""
        instruction = example.get("instruction", "").strip()
        input_text = example.get("input", "").strip() or "No additional input"
    ...
    def _compute_reward(self, example: Dict, prediction: str) -> float:
        """评审模型给出奖励得分"""
        judge_prompt = self._format_judge_prompt(example, prediction)
        reward = self._score_with_classifier(self.judge_model, self.judge_tokenizer, judge_prompt)
        return reward
    ...
    def _collect_and_save_projected_gradients(self,
        ...
        if self.accelerator.is_main_process and start_count > 1:
            logger.info(f"NICEJudgeSelector: Resuming from sample index {start_count}.")
